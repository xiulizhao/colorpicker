(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.colorpicker) {
      window.wwload.colorpicker = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/color-picker/js/color_change"], function () {
        window.wwload.colorpicker = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.colorpicker === "wait") {
      setTimeout(function () {
  ...
  function color_picker($ele) {

    var colorpickerid = $ele.attr("data-colorpickerid");
    var defaultcolor = $ele.attr("data-defaultcolor") || "#ff0000";
    $ele.find(".picker").attr("id", colorpickerid);
    ...
    var a = Colorpicker.create({
      ...
      change: function (elem, hex) {
        ...
        $.wwclass.helper.updateProp($ele, "data-x-colorpicker", hex);
        $.wwclass.helper.anijsTrigger($ele, "colorpickersuccess");
